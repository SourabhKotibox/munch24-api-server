...
echo "Raising Nginx upload limit to 20G..."

MAIN_CONF="/etc/nginx/nginx.conf"
if [ -f "$MAIN_CONF" ]; then
  if grep -q "client_max_body_size" "$MAIN_CONF"; then
    sudo sed -i 's/client_max_body_size .*/client_max_body_size 20G;/' "$MAIN_CONF"
  else
    sudo sed -i '/http {/a \    client_max_body_size 20G;' "$MAIN_CONF"
  fi
fi
...
for SITE_CONF in "${SITE_CONFS[@]}"; do
  ...
  if grep -q "client_max_body_size" "$SITE_CONF"; then
    sudo sed -i 's/client_max_body_size .*/client_max_body_size 20G;/' "$SITE_CONF"
  else
    sudo sed -i '/server {/a \    client_max_body_size 20G;' "$SITE_CONF"
  fi

  if ! grep -q "proxy_request_buffering off;" "$SITE_CONF"; then
    sudo sed -i '/proxy_pass/a \        proxy_buffering off;\n        proxy_request_buffering off;\n        proxy_read_timeout 3600s;\n        proxy_send_timeout 3600s;\n        client_max_body_size 20G;' "$SITE_CONF"
  fi
done
